...
public class Polynomial_Driver_Extensions
{
   public static void main(String[] args)
   {
      Polynomial poly = new Polynomial();    // 3x^7 + 2x - 1
      poly.makeTerm(7, 3);
      poly.makeTerm(1, 2);
      poly.makeTerm(0, -1);
      
      System.out.println("Derivative(Poly): " + poly.findDerivative()); // 21x^6 + 2
      
      Polynomial poly2 = new Polynomial();    // 21x^6 + 2
      poly2.makeTerm(6, 21);
      poly2.makeTerm(0, 2);
      
      System.out.println("Integral(Poly2): " + poly2.findIntegral());
      
      double d = poly.findRoot(poly);
      if(d != -1.000001)
         System.out.println("Roots of Poly: ~" + d);  
      else
         System.out.println("Roots of Poly: no known roots in the range of (-5, 5)");
      
      System.out.println(poly.taylorPoly_eX());  
      System.out.println(poly.taylorPoly_sinX()); 
      System.out.println(poly.taylorPoly_eXsinX()); 
      System.out.println(poly.approxDeriv_eXsinX());
      System.out.println(poly.approxInteg_eXsinX());
      
      System.out.println("Poly: " + poly.toString());
      System.out.println("Poly's " + poly.findMin(poly, -1, 1));
      System.out.println("Poly's " + poly.findMax(poly, -1, 1));
   }
}
...
interface PolynomialInterface
{
   ...
   public double findRoot(Polynomial other); //default range: (-5, 5)
   ...
   public String taylorPoly_eX();
   public String taylorPoly_sinX();
   public String taylorPoly_eXsinX();
   public String taylorPoly_eXcosX();
   public String approxDeriv_eXsinX();
   public String approxInteg_eXsinX();
}
...
class Polynomial implements PolynomialInterface
{
   ...
   public Polynomial(String expr)
   {
      ...
      for( String term : terms )
      {
         ...
         {  
            if(term.contains("x^"))
            {
               double term1 = Double.parseDouble(term.substring(termLengthMinusOne, termLength));
               double term2 = 0;
               ...
               if(term.length() > 4)
               {
                  if(!term.contains("-"))
                  {
                     if(term.substring(0, term.indexOf('x')).length() != 0)   
                        term2 = Double.parseDouble(term.substring(0, term.indexOf('x')));
                  }
                  
                  else
                  {
                     term2 = Double.parseDouble(term.substring(term.indexOf('-') + 1, term.indexOf('x')));
                     term2 *= -1;
                  }
               }
            ...
            else
            {
               ...
               else if(term.contains("x"))
               {
                  ...
                  if(term.length() != 1)
                     term2 = Double.parseDouble(term.substring(0, termLengthMinusOne));
               ...
               {
                  double term2 = Double.parseDouble(term);
                  makeTerm(0.0, term2);
               }
            }
         }
   ...
   public String taylorPoly_eX()
   {
      String toReturn = "Taylor Polynomial of e^x: ";
      ...
      for(int i = 9; i >= 3; i--)
      {
         ...
         double toRetFrac = 1.0 / factProd;
         toReturn += df.format(toRetFrac) + "x^" + i + "+";
      }
   ...
   public String taylorPoly_sinX()
   {
      String toReturn = "Taylor Polynomial of sin(x): ";
      DecimalFormat df = new DecimalFormat("0.0000000000");
      int iter = 0;
   ...
   public String taylorPoly_eXsinX()
   {
      String toReturn = "Taylor Polynomial of e^x*sin(x): ";
   ...
   public String taylorPoly_eXcosX()
   {
      String toReturn = "Taylor Polynomial of e^x*cos(x): ";
      DecimalFormat df = new DecimalFormat("0.00000000000");
   ...
   public String approxDeriv_eXsinX()  //e^x*sin(x) + e^x*cos(x)
   {
      String toReturn = "Approximate derivative of e^x*sin(x): ";
      Polynomial poly = new Polynomial();
      String str1 = poly.taylorPoly_eXsinX();
      String str2 = poly.taylorPoly_eXcosX();
      
      str1 = str1.substring(33);
      str2 = str2.substring(33);
      
      Polynomial eXsinX = new Polynomial(str1);
      Polynomial eXcosX = new Polynomial(str2);

      toReturn += "" + eXsinX.add(eXcosX);
      return toReturn;
   }
   
   public String approxInteg_eXsinX()  //(e^x*sin(x) - e^x*cos(x)) / 2
   {
      String toReturn = "Approximate integral of e^x*sin(x): ";
      Polynomial poly = new Polynomial();
      String str1 = poly.taylorPoly_eXsinX();
      String str2 = poly.taylorPoly_eXcosX();
      ...
      Polynomial polyMinusOne = new Polynomial();
      polyMinusOne.makeTerm(0, -1);
      
      Polynomial minus_eXcosX = eXcosX.multiply(polyMinusOne);
      Polynomial eXsinXAdd = eXsinX.add(minus_eXcosX);
      
      Polynomial divideByTwo = new Polynomial();
      divideByTwo.makeTerm(0.0, 0.5);
      
      Polynomial eXcosXAdd = eXsinXAdd.multiply(divideByTwo);
