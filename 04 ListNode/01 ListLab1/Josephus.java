...
public class Josephus
{
   ...
   {
      /*ListNode tempInsert = p;         //creates copy of node p
      
      if(tempInsert.getNext() != p)
         while(tempInsert.getNext() != p)
            tempInsert = tempInsert.getNext();
      
      ListNode finalTemp = new ListNode(obj, p);
      tempInsert.setNext(finalTemp);*/
   ...
   public static ListNode remove(ListNode p, int count)
   {
     /*if(count == 1)
      ...
      for(int i = 0; i < count - 1; i++)
         tempRO = tempRO.getNext();
         
      tempRO.setNext(tempRO.getNext().getNext());
