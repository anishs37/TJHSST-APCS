...
/* NOTE: In Codepost, please use the file "LunchRoom Seniors First" as a reference,
   and NOT the other text file. Thank you.
*/

//MyPriorityQueue is extension class.
...
public class LunchRoom
{
   ...
   public static int thisCustomersGrade;
   public static String gradeLevelToPrint;
   ...
   /* This Math.random() will return one of four possible values:
      0 will represent a senior entering the queue
      1 will represent a junior entering the queue
      2 will represent a sophomore entering the queue
      3 will represent a freshman entering the queue
   */
   
   public static int gradeToBeServed()
   {
      return (int)(Math.random() * 4);
   }
   
   public static void displayTimeAndQueue(MyPriorityQueue<Customer> q, int min)
   {
   ...
   static class Customer implements Comparable<Customer>
   {
      ...
      public Customer(int time, int grade)
      {
         arrivedAt = time;
         classNum = grade;
         
         gradeLevel = new String[4];
         gradeLevel[0] = "Se";
         gradeLevel[1] = "Ju";
         gradeLevel[2] = "So";
         gradeLevel[3] = "Fr";
      ...
      public String getGradeLevelToPrint()
      {
         String str = gradeLevel[classNum];
         return str;
      }
      
      public int compareTo(Customer other)
      {
         int classNumInt = this.getGradeLevel();
         int otherNumInt = other.getGradeLevel();
         
         int toCompare = (new Integer(classNumInt)).compareTo(new Integer(otherNumInt));
         
         if(toCompare != 0)
            return toCompare;
         
         return arrivedAt - other.getArrived();
      }
      
      public String toString()
      {
         String str = arrivedAt + ":" + gradeLevel[classNum];
         return str;
      }
   }
   ...
   {
      String[] classes = new String[4];           //order: senior, junior, sophomore, freshman
      int[] served = new int[4];                  //order: senior, junior, sophomore, freshman
      int[] lWait = new int[4];                   //order: senior, junior, sophomore, freshman
      int[] tWait = new int[4];                   //helper method to find average wait time per grade
                                                  //order: senior, junior, sophomore, freshman
      ...
      MyPriorityQueue<Customer> cust = new MyPriorityQueue<Customer>();
      int waitTime;
      int gradeLevel;
      String gradeLevelBeingServed = "";
      MyPriorityQueue<String> gradeLevels = new MyPriorityQueue<String>();
      int[] custServed = new int[2];                    //[0] represents cust being served
                                                        //[1] represents wait time of cust being served
      ...
      for(int i = 0; i < TIME; i++)
      {
         ...
         {
            gradeLevel = gradeToBeServed();
            waitTime = timeToOrderAndBeServed();
            cust.add(new Customer(i, gradeLevel));  
            served[gradeLevel] = served[gradeLevel] + 1;
            outfile.println("new customer min#" + i);
            String gradeLevelBeingS = "";
            ...
            { 
               custServed[0] = i;
               custServed[1] = waitTime;
               cust.remove();
               gradeLevelBeingServed = gradeLevelBeingS;
            }
            
            else
            {
               if(gradeLevelBeingS.equals("Se"))
                  gradeLevels.add("A");
               
               else if(gradeLevelBeingS.equals("Ju"))
                  gradeLevels.add("B");
               
               else if(gradeLevelBeingS.equals("So"))
                  gradeLevels.add("C");
               
               else if(gradeLevelBeingS.equals("Fr"))
                  gradeLevels.add("D");  
            }
         }
         ...
         if(custServed[1] == 0)
         {
            int numRemove = custServed[0];
            thisCustomersTime = i - custServed[0];
            gradeLevelToPrint = gradeLevelBeingServed;
            
            if(gradeLevelToPrint.equals("Se"))
               thisCustomersGrade = 0;
            
            else if(gradeLevelToPrint.equals("Ju"))
               thisCustomersGrade = 1;
            
            else if(gradeLevelToPrint.equals("So"))
               thisCustomersGrade = 2;
            
            else if(gradeLevelToPrint.equals("Fr"))
               thisCustomersGrade = 3;
                           
            outfile.println("Customer# " + numRemove + ":" + gradeLevelToPrint + " leaves and his/her total wait time is " + thisCustomersTime);
            
            if(thisCustomersTime > lWait[thisCustomersGrade])
               lWait[thisCustomersGrade] = thisCustomersTime;
            
            tWait[thisCustomersGrade] += thisCustomersTime;
            ...
            if(!cust.isEmpty())
            {
               ...
               custServed[0] = numPeek;
               custServed[1] = waitTime;             
               gradeLevelBeingServed = gradeLevels.remove();
               
               if(gradeLevelBeingServed.equals("A"))
                  gradeLevelBeingServed = "Se";
               
               else if(gradeLevelBeingServed.equals("B"))
                  gradeLevelBeingServed = "Ju";
                  
               else if(gradeLevelBeingServed.equals("C"))
                  gradeLevelBeingServed = "So";
               
               else if(gradeLevelBeingServed.equals("D"))
                  gradeLevelBeingServed = "Fr";
                  
               str = "\t" + "ServiceArea#1 " + custServed[0] + ":" + gradeLevelBeingServed + ":" + custServed[1];
               custServed[1] = custServed[1] - 1;
               cust.remove();
            }
      ...
      if((!cust.isEmpty()) || (custServed[0] != -1))
      {
         do
         {
            String str = "";
            ...
            {
               int numRemove = custServed[0];
               thisCustomersTime = k - custServed[0];
               gradeLevelToPrint = gradeLevelBeingServed;
         ...
         while((!cust.isEmpty()) || (custServed[0] != -1));
      }
      ...
      for(int x = 0; x < 4; x++)
      {
         System.out.println(classes[x] + "\t\t\t" + served[x] + "\t\t\t" + lWait[x] + "\t\t\t" + ((double)tWait[x]/served[x]));
         outfile.println(classes[x] + "\t\t\t" + served[x] + "\t\t\t" + lWait[x] + "\t\t\t" + ((double)tWait[x]/served[x]));
      }
   } 
}
